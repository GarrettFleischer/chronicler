...
import { Nodes, INSERT, REMOVE, UPDATE, LABEL, CHOICE } from "./nodes";
import "./methods";
  ...
  describe("Nodes", () => {
    describe("methods", () => {
      ...
      it("can insert owned node", () => {
        ...
        assert.equal(Nodes.find().count(), 4);
      });

      it("cannot insert unowned node", () => {
        // Run the method with `this` set to the fake invocation
        assert.throws(
          () =>
      ...
      it("cannot delete unowned node", () => {
        // Run the method with `this` set to the fake invocation
        assert.throws(
          () => remove.apply(invocation, [unownedNodeId]),
          notAuthorized
        );
      ...
      it("cannot update node parentId of non-label", () => {
        ...
        assert.throws(
          () =>
            update.apply(invocation, [choiceNodeId, { parentId: newParentId }]),
          noModify
        );
      ...
      it("cannot update unowned node text", () => {
        ...
        assert.throws(
          () => update.apply(invocation, [unownedNodeId, { text: newText }]),
          notAuthorized
        );
      ...
      it("cannot update node type", () => {
        ...
        assert.throws(
          () => update.apply(invocation, [ownedNodeId, { type: newType }]),
          noModify
        );
        const updatedComponent = Nodes.findOne({ _id: ownedNodeId });
